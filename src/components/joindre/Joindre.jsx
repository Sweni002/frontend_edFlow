 ...
 const [open, setOpen] = React.useState(false);
  ...
  useEffect(() => {
    ...
    const socket = new WebSocket(
      `ws://localhost:8000/ws/chat/${sessionId}/${userId}`
    );
    ...
    socket.onmessage = (event) => {
      try {
        ...
        if (event.data === "__fichier_ajoute__") {
          console.log("📥 Fichier ajouté détecté - actualisation fichiers...");
          fetchFiles(); // ⬅️ recharge la liste des fichiers
        } else {
          setMessages((prev) => [
            ...prev,
            { user_id: "Système", message: event.data },
          ]);
        }
      }
    };
    ...
    const fetchMessages = async () => {
      ...
      try {
        ...
        const response = await fetch(
          `http://127.0.0.1:8000/comments/${sessionId}`,
          {
            headers: {
              Authorization: `Bearer ${token}`,
            },
    ...
    <h1>ecourses</h1>
          ...
          <div className="menu">
            ...
            <div
              ...
              onClick={docFile}
            >
              <Badge count={sessionFiles.length} size="small" offset={[6, -2]}>
                <i
  ...
  <strong style={{ color: "cyan", marginRight: 4 }}>
    @{msg.user_name ? msg.user_name : "Anonyme"}:
  </strong>
)}
        ...
        <div style={{ width: "100%", height: "100%", position: "relative" }}>
          ...
          <div className="controle">
            <div className="autres">
              ...
              <i
                className={
                  camOn ? "fa-solid fa-video" : "fa-solid fa-video-slash"
                }
                onClick={toggleCam}
